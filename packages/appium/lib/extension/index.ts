...
async function importExtensions(
  ...
  const extPairs = await asyncmap(
    ...
    async (extName) => {
      ...
      try {
        ...
        return [extClass as ExtClass<ExtensionType>, extName] as [ExtClass<ExtensionType>, string];
      } catch (err: any) {
        log.error(
  ...
  return extPairs.filter((pair): pair is [ExtClass<ExtensionType>, string] => pair !== undefined);
}
